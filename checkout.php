...
.pay-card { border:2px solid #e5e7eb; border-radius:16px; padding:14px 10px; cursor:pointer; transition:all .2s; text-align:center; }
...
<div style="max-width:560px;margin:0 auto;padding:16px;">
  ...
  <div style="background:#fff;border-radius:18px;padding:16px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.06);">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
      <div class="pay-card" id="pc-UPI"  onclick="selPay('UPI')"><div style="font-size:26px;margin-bottom:6px;">📲</div><div style="font-weight:700;font-size:13px;">UPI</div><div style="font-size:11px;color:#9ca3af;margin-top:2px;">GPay·PhonePe</div></div>
      <div class="pay-card" id="pc-CARD" onclick="selPay('CARD')"><div style="font-size:26px;margin-bottom:6px;">💳</div><div style="font-weight:700;font-size:13px;">Card</div><div style="font-size:11px;color:#9ca3af;margin-top:2px;">Debit/Credit</div></div>
      <div class="pay-card" id="pc-COD"  onclick="selPay('COD')"><div style="font-size:26px;margin-bottom:6px;">💵</div><div style="font-weight:700;font-size:13px;">Cash</div><div style="font-size:11px;color:#9ca3af;margin-top:2px;">Pay at counter</div></div>
    </div>
  ...
  <div id="cardPanel" style="display:none;background:#fff;border-radius:18px;padding:16px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.06);">
    <div style="display:flex;align-items:center;gap:12px;background:#eff6ff;border-radius:12px;padding:14px;">
      ...
      <div><div style="font-weight:600;font-size:14px;color:#111;">Card Payment</div><div style="font-size:12px;color:#6b7280;margin-top:2px;">Pay ₹<?= $total ?> at the counter with your card</div></div>
    </div>
  </div>
  ...
  <div id="codPanel" style="display:none;background:#fff;border-radius:18px;padding:16px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.06);">
    <div style="display:flex;align-items:center;gap:12px;background:#f0fdf4;border-radius:12px;padding:14px;">
      ...
      <div><div style="font-weight:600;font-size:14px;color:#111;">Cash Payment</div><div style="font-size:12px;color:#6b7280;margin-top:2px;">Pay ₹<?= $total ?> in cash at the counter</div></div>
    </div>
  </div>
...
function selPay(m) {
  ...
  ['UPI','CARD','COD'].forEach(k => {
    ...
    document.getElementById({ UPI:'upiPanel', CARD:'cardPanel', COD:'codPanel' }[k]).style.display = 'none';
  });
  document.getElementById('pc-'+m).classList.add('sel');
  document.getElementById({ UPI:'upiPanel', CARD:'cardPanel', COD:'codPanel' }[m]).style.display = 'block';
...
function placeOrder() {
  ...
  if (method === 'UPI') {
    ...
    if (!utr) { alert('Please enter the UTR / Transaction ID after making UPI payment.'); return; }
    if (!confirm('Confirm order · ₹<?= $total ?> via UPI?')) return;
    window.location.href = `place_order.php?mode=UPI&utr=${encodeURIComponent(utr)}&type=${encodeURIComponent(orderType)}&area=${encodeURIComponent(area)}&address=${encodeURIComponent(address)}&landmark=${encodeURIComponent(landmark)}`;
  } else {
    if (!confirm(`Confirm order · ₹<?= $total ?> by ${method}?`)) return;
    window.location.href = `place_order.php?mode=${method}&type=${encodeURIComponent(orderType)}&area=${encodeURIComponent(area)}&address=${encodeURIComponent(address)}&landmark=${encodeURIComponent(landmark)}`;
  }
}
</script>
</body>
