...
if (!empty($_SESSION['mobile_verified'])) { header('Location: home.php'); exit; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
body { background:linear-gradient(160deg,#1a0a00,#2d1200,#1a0500); display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:100vh; text-align:center; padding:32px 24px; }
@keyframes floatY { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-14px)} }
.anim { animation:floatY 3.2s ease-in-out infinite; }
@keyframes fadeUp { from{opacity:0;transform:translateY(24px)} to{opacity:1;transform:translateY(0)} }
.fu1{animation:fadeUp .6s .1s both} .fu2{animation:fadeUp .6s .25s both}
.fu3{animation:fadeUp .6s .4s both} .fu4{animation:fadeUp .6s .55s both}
</style>
</head>
<body>
  ...
  <div class="fu4" style="width:100%;max-width:300px;">
    <a href="index.php" style="display:block;width:100%;padding:17px;border-radius:16px;background:linear-gradient(135deg,#e65c00,#f9a84d);color:#fff;font-weight:700;font-size:17px;text-decoration:none;box-shadow:0 8px 32px rgba(230,92,0,.45);">
