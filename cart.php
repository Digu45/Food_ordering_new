...
<body>
  ...
  <div style="max-width:560px;margin:0 auto;padding:16px;">
    ...
    <?php else: ?>
      ...
      <div style="background:#fff;border-radius:18px;padding:16px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.06);">
        ...
        <div id="dineinSection" style="margin-top:14px;border-top:1px solid #f3f4f6;padding-top:14px;">
          <label style="font-size:12px;font-weight:600;color:#374151;display:block;margin-bottom:5px;">
            <i class="fas fa-chair" style="color:#e65c00;margin-right:5px;"></i>Table Number <span style="color:#dc2626;">*</span>
          </label>
          <input type="number" id="inp_table" class="inp" placeholder="e.g. 5" min="1" max="100"
            style="width:100%;max-width:180px;font-size:18px;font-weight:700;text-align:center;padding:12px;" />
          <p style="font-size:11px;color:#9ca3af;margin-top:5px;">Enter the table number shown on your table</p>
        </div>
  ...
  <script>
    ...
    function selectType(t) {
      ...
      document.getElementById('dineinSection').style.display = t === 'Dine-in' ? 'block' : 'none';
      document.getElementById('addressSection').style.display = t === 'Takeaway' ? 'block' : 'none';
    }

    function goCheckout() {
      if (orderType === 'Dine-in') {
        const table = document.getElementById('inp_table').value.trim();
        if (!table) {
          alert('Please enter your Table Number.');
          document.getElementById('inp_table').focus();
          return;
        }
        window.location.href = 'checkout.php?type=Dine-in&table=' + encodeURIComponent(table);
      } else {
        const area = document.getElementById('inp_area').value.trim();
        const addr = document.getElementById('inp_address').value.trim();
        if (!area || !addr) {
          alert('Please enter your Area and Address for Takeaway.');
          return;
        }
    ...
    function recalc() {
      ...
      document.querySelectorAll('.cart-row').forEach(r => {
        const p = parseFloat(r.dataset.price),
          q = parseInt(r.querySelector('.qty-val').textContent);
        r.querySelector('.item-total').textContent = '₹' + (p * q).toFixed(2);
        sub += p * q;
      });
      const cgst = sub * CGST / 100,
        sgst = sub * SGST / 100,
        grand = sub + cgst + sgst,
        rnd = Math.round(grand);
      ...
      if (pa) pa.textContent = rnd;
    }

    function incQty(btn) {
      const row = btn.closest('.cart-row'),
        el = row.querySelector('.qty-val');
      const nq = parseInt(el.textContent) + 1;
      el.textContent = nq;
      dbQty(row.dataset.id, nq);
      recalc();
    }
    ...
    function decQty(btn) {
      ...
      dbQty(row.dataset.id, cur - 1);
      recalc();
    }

    function rmItem(row) {
      const fd = new FormData();
      fd.append('menu_id', row.dataset.id);
    ...
    function openInstr(id, cur) {
      document.getElementById('instrId').value = id;
      document.getElementById('instrText').value = cur;
    ...
    function saveInstr() {
      const fd = new FormData();
      fd.append('menu_id', document.getElementById('instrId').value);
      fd.append('instruction', document.getElementById('instrText').value);
